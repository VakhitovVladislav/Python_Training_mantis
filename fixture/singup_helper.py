...
class SingupHelper:
    ...
    def new_user(self, username, email, password):
        ...
        wd.find_element_by_xpath(u"//input[@value='Зарегистрироваться']").click()
        wd.find_element_by_link_text("Продолжить").click()

        mail = self.app.email.get_mail(username, password, "[MantisBT] Account registration")
    ...
    def extract_confirmation_url(self, text):
        return re.search(str("http://.* $", text, re.MULTILINE)).group(0)
